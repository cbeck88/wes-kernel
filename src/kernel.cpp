...
public:
	...
	void set_external_log(std::ostream * ext) const {
		log_.external_log_ = ext;
	}

	std::string log() const {
		return log_.log_.str();
	}
...
private:
	...
	std::string my_name() {
		return "wesnoth-kernel v 0.0.0, (Eris Lua 5.2.3)";
	}
...
kernel::impl*& get_kernel_impl(lua_State *L)
{
	return *reinterpret_cast<kernel::impl**>(reinterpret_cast<char*>(L) - WESNOTH_KERNEL_OFFSET);
}
...
kernel::kernel(kernel::Ctor_it begin, kernel::Ctor_it end)
	: impl_(new kernel::impl(begin, end))
	, ref_count_(0)
{
}
...
kernel::event_result kernel::fire_event(const std::string &) { return {}; }
kernel::event_result kernel::do_command(const config &) { return {}; }
kernel::event_result kernel::execute_ai_turn() { return {}; }
kernel::event_result kernel::end_turn() { return {}; }

int kernel::turn_number() const { return 0; }
int kernel::current_side_playing() const { return 0; }
int kernel::nteams() const { return 0; }
bool kernel::can_end_turn() const { return true; }
kernel::PHASE kernel::get_phase() const { return kernel::INITIAL; }
kernel::SIDE_RESULT kernel::get_side_result(int side) const { return NONE; }
kernel::CONTROLLER kernel::get_side_controller(int side) const { return EMPTY; }

bool kernel::is_on_map(map_location loc) const { return true; }
bool kernel::is_adjacent(map_location loc1, map_location loc2) const { return true; }

bool kernel::is_fogged(map_location loc, int viewing_team) const { return true; }
bool kernel::is_shrouded(map_location loc, int viewing_team) const { return true; }

config kernel::read_report(const std::string & name, int viewing_team) const { return config(); }
config kernel::evaluate(const std::string & prog, int viewing_team) const { return config(); }
